...
from neurite_pytorch.utils import resize
...
def draw_perlin(out_shape,
    ...
    for scale in scales:
        ...
        std = torch.rand(1, generator=gen, dtype=dtype)
        # Takes the shape of the mean
        gauss = torch.normal(torch.zeros(tuple(sample_shape)), std=std, generator=gen)

        zoom = [o / s for o, s in zip(out_shape, sample_shape)]
        out += gauss if scale == 1 else resize(gauss, zoom[:-1])

    return torch.permute(out, (3,0,1,2))
